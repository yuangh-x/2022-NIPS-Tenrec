...
class BERT4infaccModel(nn.Module):
    ...
    def forward(self, x, policy_action):#, pos, neg
        # x = self.bert(x)
        mask = (x > 0).unsqueeze(1).repeat(1, x.size(1), 1).unsqueeze(1)
    ...
    def predict(self, x, policy_action):  # , pos, neg
        # note: batch_size = 1
        # print("--------", item_seq.max())
        mask = (x > 0).unsqueeze(1).repeat(1, x.size(1), 1).unsqueeze(1)
        dilate_input = self.bert.embedding(x)
...
class Bert_PolicyNetGumbel(nn.Module):
    def __init__(self, args):
        super(Bert_PolicyNetGumbel, self).__init__()
